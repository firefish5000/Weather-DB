...
sub GetForecast() { # Parses Extended Forcast
	...
	$tfc = {
		LastUpdated=>time,
		Current=>{0=>$FC->GetCurrent($Loc)},
		Minutes=>$FC->GetMinutes($Loc),
		Hours=>$FC->GetHours($Loc),
		Days=>$FC->GetDays($Loc),
	};
	#Info Dumper $FC;
	store($FC, $FC->{STORE_FILE}); # FIXME
	return $FC;
}
...
sub GetDays() {
	...
	for my $i (1..45) {
		...
		my $day = Daily_Parser(Location=>$Loc,%$args,day=>$i);
		$tfc->{$day->{Time}}=$day;
	}
	return $tfc;
}
...
sub Daily_Parser() {
	...
	my $Loc = $FC->Location(shift); # TODO/FIXME
	my $day=$args->{day}//1;
	my $Root=Build_Html_Tree( $Loc, 'daily-weather-forecast',"?day=$day" );
	my $tfc = {}; # ForCast
...
sub Current_Parser() {
	my $FC = shift;
	my $Loc = $FC->Location(shift); # TODO/FIXME
	my $Root=Build_Html_Tree($Loc, 'current-weather');
	my $tfc = {}; # ForCast
	$tfc->{Time}=0;
	$tfc->{Range}=0;
	$tfc->{LastUpdated}=time;
...
sub Advisories_Parser() { # TODO
	my $FC = shift;
	my $Loc = $FC->Location(shift); # TODO/FIXME
	my $Root=Build_Html_Tree( $Loc->{Path} . '/weather-warnings/' . $Loc->{ID} );
}
sub Stats_Parser() {
	my $Stats=shift;
	my $tfc={};
